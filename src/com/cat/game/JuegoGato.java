...
public class JuegoGato {
	...
	public JuegoGato() {
		...
		jugadorUno = 'X';
		jugadorDos = 'O';
		turno = true; // Si El Valor De "turno" Es Igual A "true" El Turno Es Del Jugador 1, De Lo
						// Contrario Es Jugador 2.
		 contador = 0; // Cuenta Las Veces Que Han Participado Los Jugadores 1 y 2.
		inicializarTablero();
	}

	// Inicializa El Tablero y  Especifica El Numero Que Le Corresponde A Cada Casilla.
	...
	// Indica La Posicion Donde Se Ingresara El Dato Dentro Del Arreglo Por Medio De
	// Parametros.
	public void indicarPosicionArreglo(int a, int b, char c) {
		tablero[a][b] = c;
	}

	// Modifica el valor De La Variable "turno".
	public void setTurno(boolean d) {
		turno = d;
	}

	// Modifica el valor De La Variable "contador".
	public void setContador(int e) {
		contador = e;
	}

	// Devuelve el Valor De La Posicion Del Arreglo Indicada Por Medio De Parametros.
	public char getPosicionArreglo(int a, int b) {
		return tablero[a][b];
	}

	// Devuelve el valor De La Variable "jugadorUno".
	public char getJugadorUno() {
		return jugadorUno;
	}

	// Devuelve el valor De La Variable "jugadorDos".
	public char getJugadorDos() {
		return jugadorDos;
	}

	// Devuelve el valor De La Variable "turno".
	...
	// Imprime e Indica El El Turno Del Jugador Que Le Toca Elejir Una Casilla Con
	// Ayuda del Metodo "getTurno()".
	...
	// Devuelve el Caracter De Tipo Char Para Posteriormente Ingresarlo A Alguna
	...
	// Cambia El Valor De La Variable "turno" Para Llevar El Orden Correcto De Los
	// Turnos Entre Los 2 Jugadores.
	public void cambiarTurno() {

		if (obtenerFigura() == 'X') // Si el Metodo regresa un valor char igual a "X" cambia el valor de "turno" a
									// "false".
	...
	// Comprueba Si La Posicion Del Arreglo Al Que El Jugador Desea Ingresar El
	// Caracter No Fue Utilizada Antes.
	public void comprobarEspacio(char espacio, int posicionUno, int posicionDos) {

		// Si Se Cumple La Condicion Se Ingresa El Valor En La Posicion Del Arreglo
		// Especificada.
		if (espacio != getJugadorUno() && espacio != getJugadorDos()) {

			indicarPosicionArreglo(posicionUno, posicionDos, obtenerFigura());// Ingresa EL Caracter Si Se Cumple La
																				// Condicion.
			...
			// Imprime El Siguiente Mensaje En Caso De Que No Se Cumpla La Anterior
			// Condicion.
			System.out.println("La Casilla Esta Ocupada, Elija Otra Por Favor");
	}

	// Metodo Que Hace Uso De Un "switch" Para Indicar La Posicion Del Arreglo Que
	// El Jugador Elijio En El Juego.
	public void elegirPosicion(int posicion) {

		switch (posicion) {

		// Comprueba Si La Posicion Esta Disponible Valiendose Del Metodo
		// "comprobarEspacio()"
		case 1:
		...
		case 9:
			...
			break;
		default:
			// Imprime El Siguiente Mensaje En Caso De Que El "switch" No Reciba Un Digito
			// Del 1 al 9.
	...
	// Solo Imprime Un Mensaje Distinto Dependiendo Del Jugador Que Haya Resultado
	// Ganador.
	...
	// Verifica Las Unicas 8 Combinaciones Que Pueden Dar La Victoria A Uno De Los
	// Jugadores.
	public void buscarGanador() {

		// Ciclo Con Instrucciones Para Comprobar Las 3 Posibilidades De Ganar De Forma
		// Horizontal.
		for (int i = 0; i < 3; i++) {
		...
		// Ciclo Con Instrucciones Para Comprobar La Unica Posibilidade De Ganar De
		// Forma Diagonal Inversa.
		...
		do {
			...
			try {
				...
				if (getTurno() == true)
					...
					System.out
							.println("ERROR!: Ingreso Un Caracter No Valido, Por Favor, Intente Nuevamente JUGADOR 1.");

				else

					System.out
							.println("ERROR!: Ingreso Un Caracter No Valido, Por Favor, Intente Nuevamente JUGADOR 2.");
			}
